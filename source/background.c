...
int background_functions(
  ...
  if (return_format == pba->long_info) {
    ...
    //pvecback[pba->index_bg_rho_m] = rho_m
    /*  */
    /*  */
...
int background_init(
  ...
  if (pba->background_verbose > 0) {
    ...
    printf("Computing background\n");
      ...
      if (pba->has_MG == _TRUE_){
          printf("The value of Omega0_T is %0.4f and beta parameter is %0.8f\n", pba->Omega0_T, beta(pba));}
...
//------- f(T) = T + alpha * (-T)^p
//double beta(struct background *pba){
//    return pow(6.0 * pow(pba->H0, 2.0), 1.0 - pba->p) * (pba->Omega0_T)/(2.0 * pba->p - 1.0);
//}
//double fE(struct background *pba, double rE){
//    return -6.0 * pow(rE,2) * pow(pba->H0, 2) + beta(pba) *  pow(6.0 * pow(rE, 2) * pow(pba->H0, 2), pba->p);
//}
//
//double EdfE(struct background *pba, double rE){
//    return 12.0 * pow(pba->H0,2.0) * pow(rE, 2.0) * (beta(pba) * pba->p * pow(6.0 * pow(rE,2) * pow(pba->H0,2), pba->p - 1.0) - 1.0);
//}
