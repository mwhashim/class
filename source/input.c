...
//             "In input file, either Omega_Lambda or Omega_fld must be left unspecified, except if Omega_scf is set and <0.0, in which case the contribution from the scalar field will be the free parameter.");
  ...
  class_test((flag1 == _TRUE_) && (flag2 == _TRUE_) && (flag3 == _TRUE_) && ((flag4 == _FALSE_) || (param4 >= 0.)),
               errmsg,
               "In input file, either Omega_Lambda, Omega_fld or Omega_scf must be left unspecified, except if Omega_T is set and <0.0, in which case the contribution from the scalar field will be the free parameter.");
...
//  }
      ...
      if (input_verbose > 0) printf(" -> matched budget equations by adjusting Omega_scf = %e\n",pba->Omega0_scf);
        ...
        if (input_verbose > 0) printf(" -> matched budget equations by adjusting Omega_T = %e\n", pba->Omega0_T);
    }
  ...
  */
  ...
  if (pba->Omega0_T !=0.){
      class_read_double("p",pba->p);
      } // read MG  free parameter
